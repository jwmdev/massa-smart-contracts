...
class WalletClient extends BaseClient_1.BaseClient {
    ...
    static async walletGenerateNewAccount() {
        ...
        const secretKeyBase58Encoded = SECRET_KEY_PREFIX +
            (0, Xbqcrypto_1.base58Encode)(Buffer.concat([version, secretKeyArray]));
        const secretKey = new keyAndAddresses_1.SecretKey(secretKeyBase58Encoded);
        // get public key
        const publicKey = await secretKey.getPublicKey();
        // get wallet account address
    ...
    static async getAccountFromSecretKey(secretKeyBase58) {
        // get private key
        const secretKey = new keyAndAddresses_1.SecretKey(secretKeyBase58);
        // get public key
        const publicKey = await secretKey.getPublicKey();
        // get wallet account address
        const address = new keyAndAddresses_1.Address(publicKey);
    ...
    async signMessage(data, accountSignerAddress) {
        ...
        if (!signerAccount) {
            ...
            account = new Web3Account_1.Web3Account(signerAccount, this.publicApiClient);
        }
    ...
    async verifySignature(data, signature, signerPubKey) {
        ...
        const publicKey = keyAndAddresses_1.PublicKey.fromString(signerPubKey);
        // setup the message digest.
        const bytesCompact = Buffer.from(data);
        const messageDigest = (0, Xbqcrypto_1.hashBlake3)(bytesCompact);
        try {
            // setup the signature.
            const versionAndSignatureBytes = (0, Xbqcrypto_1.base58Decode)(signature.base58Encoded);
